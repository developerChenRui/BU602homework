...
PD = {'fname':"wordplayer.py",'program':'wordplayer.py','slowfactor':1,'original':"wordplayer.py"}
#PD = {'fname':"wordplayer.cpp",'program':'wordplayer','slowfactor':1,'original':'wordplayer.cpp'}

testing = 'py' if PD['fname'].endswith('py') else 'cpp'
...
class wordplayerTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        args += [PD['program'],'short_wordlist.txt']
...
class wordplayerBigTestCase(unittest.TestCase):
    ...
    def setUp(self):
        args = ['python'] if testing=='py' else []
        args += [PD['program'],'big_wordlist.txt']
        self.process = subprocess.Popen(args,**popen_specs)
        time.sleep(0.02)
        return_code = self.process.poll()
        if return_code:
...
def test_speed():
        args = ['python'] if testing=='py' else []
        args += [PD['program'],'big_wordlist.txt']
        process = subprocess.Popen(args,**popen_specs)
        time.sleep(0.02)
        if process.poll():
            return {}

        time.sleep(2*PD['slow_factor'])
        speed_factor=[]
        for target_time, case, answer in Tests_Speed:
                start_time=time.time()
                words = ask_wordplayer(process,case)
                duration = (time.time()-start_time)/ PD['slow_factor']
        ...
        if DEBUG:
            ...
            print('==================')
            print('Time Ratio    Case')
            print('----------    ----')
            for test,speed in speed_factor:
                print("{:>10.3f}".format(speed),test)
...
def main_python():
    ...
    for spec_test in [wordplayerTestCase,wordplayerBigTestCase]:
        ...
        unittest.loader.TestLoader().loadTestsFromTestCase(spec_test).run(results)
    ...
    Grade['elegance'] = min(1,300/complexity)
    ...
    print('authors          : {}'.format(" ".join(authors)
                                               if authors else ec602lib.AUTHWARN))
...
def main_cpp():
    ...
    print("cpplint          : {}".format("{} problems".format(cpplint_count) if cpplint_count else "ok"))
...
def main_setup():
    "discover computer speed, download needed files, compile"
    st = time.time()
    D = {}
    for k in range(10000):
        D[k] = random.randint(1,100)
    en = time.time()
    PD['slow_factor'] = (en-st)/0.02
    
    if DEBUG:
        print('slow factor for your computer:',PD['slow_factor'])
    ...
    if testing == 'cpp':
        T = subprocess.run(['g++', "-std=c++14", "-O3", PD['fname'], "-o", PD['program']])
