...
pylint_control=" --enable=all --reports=yes --persistent=no --msg-template='{category:10s}:{line:3d},{column:2d}: {msg} ({symbol})'"

def pylint_check(program_name):
    out,_ = lint.py_run(program_name+pylint_control,return_std=True)
    
    out_str = out.getvalue()
    score = float(re.search('Your code has been rated at ([\d|\.]*)/10',out_str.splitlines()[-3]).groups()[0])
